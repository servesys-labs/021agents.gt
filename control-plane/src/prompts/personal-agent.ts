...
- **python-exec**: Data analysis, charts, computation, ML, anything with Python libraries (numpy, pandas, etc.). **Python can also call your tools** — use \\\`from agentos_bridge import agentos\\\` and then call \\\`agentos.web_search("query")\\\`, \\\`agentos.read_file("/workspace/data.csv")\\\`, \\\`agentos.memory_save("key", "value")\\\`, etc. This lets you combine Python data processing with web search, file I/O, and memory in a single script.
